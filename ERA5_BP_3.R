...
res <- 
  data.frame(
    ...
    tmed_total = NA, tmed_pre_1 = NA, tmed_post_1 = NA, tmed_pre_2 = NA, tmed_post_2 = NA,
    tmed_pre_3 = NA, tmed_post_3 = NA, tmed_pre_4 = NA, tmed_post_4 = NA, tmed_pre_5 = NA, tmed_post_5 = NA
  )
res <- res[-1,]
...
bp_analysis <- function(i, data) {
  ...
  for (j in 1:length(year_break)) {
    if (is.na(year_break[j])) {
      ...
      results[[paste0("tmed_pre_", j)]] <- mean(pre)
      ...
      results[[paste0("tmed_post_", j)]] <- mean(post)
    }
  }
